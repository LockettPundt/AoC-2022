const splitIntoCompartments = (backpack: string): string[] => [
...
const findCommonItem = (x: string, y: string[]): string | undefined => {
  return x
    .split(``)
    .find((curr) => y.every((str) => str.split(``).some((letter) => letter === curr)));
};
...
export const dayThreePartTwo = (data: string[]): number => {
  ...
  return splitElves.reduce(
    (sum, [packOne, packTwo, packThree]) =>
